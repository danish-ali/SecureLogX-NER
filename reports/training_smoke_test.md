...
- Weight decay: 0.01
- Warmup ratio: 0.1
- Train batch / gradient accumulation / effective batch: 2 / 2 / 4
- Eval batch: 4
...
- Train: 37/37 aligned; 0 boundary-adjusted; 0 truncated; 0 explicitly unrepresentable.
- Dev: 14/14 aligned; 0 boundary-adjusted; 0 truncated; 0 explicitly unrepresentable.

## Per-epoch dev selection

| Epoch | Train loss | Dev loss | Dev micro F1 | Dev macro F1 | High-risk recall | Optimizer steps |
|---:|---:|---:|---:|---:|---:|---:|
| 1 | 3.785000 | 3.360664 | 0.000000 | 0.000000 | 0.000000 | 2 |

## Selection

- Selected epoch: **1**
- Best checkpoint: `.tmp-training-smoke-final/best-checkpoint`
- Elapsed training/evaluation time: 0.03 minutes
...
Three independent two-optimizer-step smoke runs produced byte-identical
`model.safetensors` files. The repeated SHA-256 was
`cfeeb250ffafd637a932d533c9dce8aeece9c9498a6d7da6ce721d90c4dca1f1`.
The final replay exported `PYTHONHASHSEED=42` before interpreter startup and
used the hardened training contract. Only wall-clock timing metadata differed.

The zero smoke metrics are expected after only two optimizer steps and are not
used as model-quality evidence. The smoke cycle establishes executable,
deterministic training, evaluation, one-pass receipts, and report generation
before the full run.
