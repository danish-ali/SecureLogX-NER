...
# Frozen v1.1 partitions that can be regenerated deterministically when the
# on-disk file is unreadable (e.g. restrictive ACLs from the generating
# sandbox).  The regenerated bytes must still match the cached input hash.
REGENERABLE_V1_1_PARTITIONS = {
    Path("data/ml_v1_1/context_contrast/dev_challenge.jsonl").as_posix(): "dev_challenge",
}
EPOCH_METRICS = Path(
...
def _load_records(root: Path, record_path: Path) -> tuple[list[dict[str, Any]], str]:
    """Load frozen records, regenerating deterministically if the file is unreadable."""

    try:
        return load_jsonl(root / record_path), sha256_file(root / record_path)
    except PermissionError:
        partition = REGENERABLE_V1_1_PARTITIONS.get(record_path.as_posix())
        if partition is None:
            raise
    generator_dir = str(root / "scripts" / "data")
    if generator_dir not in sys.path:
        sys.path.insert(0, generator_dir)
    import generate_securelogx_ml_v1_1_context_contrast as v1_1_generator

    records = v1_1_generator.generate_all(seed=42)[partition]
    return records, _canonical_jsonl_sha256(records)


def _load_cached_split(
    root: Path, name: str, record_path: Path, cache_path: Path
) -> tuple[list[dict[str, Any]], list[list[dict[str, Any]]], dict[str, Any], str]:
    records, actual_input_hash = _load_records(root, record_path)
    ...
    return records, normalized, cache, actual_input_hash
...
def analyze(root: Path) -> dict[str, Any]:
    ...
    for name, (record_path, cache_path) in SPLITS.items():
        records, predictions, cache, records_sha256 = _load_cached_split(
